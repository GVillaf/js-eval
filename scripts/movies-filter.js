//datos de las peliculas y usuarios
const users = [
  {
    id: 1,
    name: "Leanne Graham",
...
// Escucha al evento de carga del DOM.
document.addEventListener("DOMContentLoaded", () => {
  // Escucha al evento de envío del formulario.
  document.querySelector("form").addEventListener("submit", (event) => {
    ...
    // Llamamos a la función 'filterMovies' con los valores obtenidos del formulario.
    filterMovies({ users, movies, userId, fromDate, toDate, rate });
  });

  // Prevenimos la entrada no numérica en el campo 'userId'.
  document.querySelector("#userId").addEventListener("input", (event) => {
    event.target.value = event.target.value.replace(/[^0-9]/g, "");
  });
});

// Capturamos el contenedor de los resultados.
...
function renderMovies(movies) {
  ...
  if (movies.length === 0) {
    ...
    message.textContent = "No movies were found that match the filters.";
    containerResults.appendChild(message);
  } else {
    let content = "";
    movies.forEach((movie) => {
      ...
      <div class="card-wrapper">
        <div class="card-text">
          <h3>Movie: ${movie.movie} - Rate: ${movie.rate}</h3>
          <ul>
            <li>User ID: ${movie.id}</li>
            <li>Username: ${movie.username}</li>
            <li>Email: ${movie.email}</li>
            <li>Address: ${movie.fullAddress}</li>
            <li>Company: ${movie.company}</li>
          </ul>
        </div>
        <div class="card-image">
          <img src="${movie.image}" alt="Poster for ${movie.movie}" width="100">
        </div>
      </div>
      `;
    });
    containerResults.innerHTML = content;
    // Scroll hasta los resultados una vez se hayan renderizado
    containerResults.scrollIntoView({behavior: "smooth"});
  }
}
// Función para filtrar las películas propiamente dicha.
function filterMovies({ users, movies, userId, fromDate, toDate, rate }) {
  let filteredUser;
  // Si se ha ingresado un ID de usuario, filtra el usuario correspondiente.
  if (userId) {
    // Si no se ha ingresado un ID de usuario, toma todos los usuarios.
    filteredUser = users.filter((user) => user.id === parseInt(userId));
  } else {
    filteredUser = users;
  }
  // Filtramos las películas según los criterios del formulario.
  ...
  // Añade los detalles del usuario a las películas filtradas.
  const moviesWithUserDetails = filteredMovies.map((movie) => {
  ...
  // Llamamos a la función 'renderMovies' para mostrar las películas filtradas con los detalles del usuario.
  renderMovies(moviesWithUserDetails);
}
